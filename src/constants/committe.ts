// import AlokRTripathy from "../../assets/committee/alok-r-tripathy.webp";
// import AshishKPradhan from "../../assets/committee/ashish-k-pradhan.webp";
// import AtalBHarichandan from "../../assets/committee/atal-harichandan.webp";
// import BBMukherjee from "../../assets/committee/b-b-mukherijee.webp";
// import BharadwajNanda from "../../assets/committee/bharadwaj-nanda.webp";
// import BikramadityaDas from "../../assets/committee/bikramaditya-das.webp";
// import BiswaBMangaraj from "../../assets/committee/biswa-b-mangaraj.webp";
// import DCRao from "../../assets/committee/d-c-rao.webp";
// import DebashreetDas from "../../assets/committee/debashreet-das.webp";
// import ManasRNayak from "../../assets/committee/manas-r-nayak.webp";
// import ManasRanjanKabat from "../../assets/committee/manas-ranjan-kabat.webp";
// import ManojKSahu from "../../assets/committee/manoj-k-sahu.webp";
// import NishiPMinz from "../../assets/committee/nishi-p-minz.webp";
// import PiyushRDash from "../../assets/committee/piyush-r-dash.webp";
// import PKMishra from "../../assets/committee/pk-mishra.webp";
// import PrakashPanda from "../../assets/committee/prakash-panda.webp";
// import SaswatSatpathi from "../../assets/committee/saswat-satapathi.webp";
// import SebatiDansena from "../../assets/committee/sebati-dansena.webp";
// import ShibaPDash from "../../assets/committee/shiba-p-dash.webp";
// import ShivKMishra from "../../assets/committee/shiv-k-mishra.webp";
// import SujitKDas from "../../assets/committee/sujit-k-das.webp";
// import SujitKKhuntia from "../../assets/committee/sujit-k-khuntia.webp";
// import SuvenduKJayasingh from "../../assets/committee/suvendu-k-jayasingh.webp";
// import MAdhusmitaBiswal from "../../assets/committee/madhusmita-biswal.webp";
// import SuvenduKRath from "../../assets/committee/suvendu-k-rath.webp";
import AmiyaKrath from "../assets/committee/Prof.-Amiya-Kumar-Rath-VC.webp";

export const coreCommitteeMembers = [
  {
    committee: "Chief Patron",
    committe_id: "chief_patron",
    members: [
      {
        name: "Prof. Amiya Kumar Rath",
        designation: "Hon'ble Vice Chancellor",
        organization: "BPUT, Odisha, Rourkela",
        image: AmiyaKrath,
      },
    ],
  },
  {
    committee: "Patron",
    committe_id: "patron",
    members: [
      {
        name: "Prof. Sivkumar Mishra",
        designation: "Director I/C",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: ShivKMishra,
      },
      {
        name: "Prof. Manas Ranjan Kabat",
        designation: "Principal",
        organization: "IMIT, Cuttack",
        // image: ManasRanjanKabat,
      },
    ],
  },
  {
    committee: "Co-Patron",
    committe_id: "co_patron",
    members: [
      {
        name: "Prof. Manas Ranjan Nayak",
        designation: "Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: ManasRNayak,
      },
    ],
  },
  {
    committee: "General Chair",
    committe_id: "general_chair",
    members: [
      {
        name: "Prof. Bibhudatta Sahoo",
        designation: "",
        // designation: "Professor",
        organization: "NIT, Rourkela, Odisha",
        // image: ManasRNayak,
      },
    ],
  },
  {
    committee: "Program Chair",
    committe_id: "program_chair",
    members: [
      {
        name: "Prof. Srinivas Sethi",
        designation: "",
        // designation: "Professor",
        organization: "IGIT Sarang, Dhenkanal, Odisha",
        // image: ManasRNayak,
      },
      {
        name: "Prof. Mufti Mahmud",
        designation: "",
        // designation: "Professor",
        organization: "KFUPM, Saudi Arabia",
        // image: ManasRNayak,
      },
    ],
  },
  {
    committee: "Organizing Chair",
    committe_id: "organizing_chair",
    members: [
      {
        name: "Mrs. Nishi Poonam Minz",
        designation: "OAS (SAG), Registrar",
        organization: "BPUT, Odisha, Rourkela",
        // image: NishiPMinz,
      },
      {
        name: "Prof. Biswa Binayak Mangaraj",
        designation: "Director Examination",
        organization: "BPUT, Odisha",
        // image: BiswaBMangaraj,
      },
      {
        name: "Prof. Sujit Kumar Khuntia",
        designation: "Director CD",
        organization: "BPUT, Odisha, Rourkela",
        // image: SujitKKhuntia,
      },
      {
        name: "Mr. Manoranjan Mohapatra",
        designation: "Director I/C P&IT",
        organization: "BPUT, Odisha, Rourkela",
        // image: SujitKDas,
      },
      {
        name: "Prof. Piyus Ranjan Das",
        designation: "Faculty",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: PiyushRDash,
      },
      {
        name: "Prof. Bikramaditya Das",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: BikramadityaDas,
      },
    ],
  },
  {
    committee: "Finance Chair",
    committe_id: "finance_chair",
    members: [
      {
        name: "Mrs. Sebati Dansena",
        designation: "Finance Officer",
        organization: "BPUT, Odisha, Rourkela",
        // image: SebatiDansena,
      },
      {
        name: "Sri Mangal Minz",
        designation: "Accounts Officer, OFS-I (JB)",
        organization: "BPUT, Odisha, Rourkela",
        // image: NishiPMinz,
      },
      {
        name: "Prof. Manas Ranjan Kabat",
        designation: "Principal",
        organization: "IMIT, Cuttack",
        // image: ManasRanjanKabat,
      },
      {
        name: "Dr. Subhendu Kumar Rath",
        designation: "Dy. Registrar",
        organization: "BPUT, Odisha, Rourkela",
        // image: SuvenduKRath,
      },
      {
        name: "Prof. D. Chandrasekhar Rao",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: DCRao,
      },
    ],
  },
  {
    committee: "Convenors",
    committe_id: "convenors",
    members: [
      {
        name: "Prof. D. Chandrasekhar Rao",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: DCRao,
      },
      {
        name: "Prof. Bikramaditya Das",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: BikramadityaDas,
      },
    ],
  },

  {
    committee: "Hospitality Chair",
    committe_id: "hospitality_chair",
    members: [
      {
        name: "Prof. Suvendra Kumar Jayashingh",
        designation: "Hospitality Chair",
        organization: "IMIT, Cuttack",
        // image: SuvenduKJayasingh,
      },
      {
        name: "Prof. Alok Ranjan Tripathy",
        designation: "Faculty",
        organization: "IMIT, Cuttack",
        // image: AlokRTripathy,
      },
      {
        name: "Prof. Shiba Prasad Dash",
        designation: "Assistant professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: ShibaPDash,
      },
      {
        name: "Prof. Madhusmita Biswal",
        designation: "Assistant professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: MAdhusmitaBiswal,
      },
      {
        name: "Prof. Saswat Satpathi",
        designation: "Assistant Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: SaswatSatpathi,
      },
    ],
  },
  {
    committee: "Logistic Chair",
    committe_id: "logistic_chair",
    members: [
      {
        name: "Prof. Manoj Kumar Sahu",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: ManojKSahu,
      },
      {
        name: "Prof. Madhusmita Biswal",
        designation: "Assistant professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: MAdhusmitaBiswal,
      },
      {
        name: "Prof. Asish Kumar Padhan",
        designation: "Assistant professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: AshishKPradhan,
      },
      {
        name: "Prof. Ramesh Kumar Sahoo",
        designation: "Assistant professor",
        organization: "IGIT Sarang, Dhenkanal, Odisha",
        // image: AshishKPradhan,
      },
    ],
  },
  {
    committee: "Accomodation Chair",
    committe_id: "accomodation_chair",
    members: [
      {
        name: "Prof. Bibhuti Bhusan Mukharjee",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha",
        // image: BBMukherjee,
      },
      {
        name: "Prof. Pradeep Kumar Mishra",
        designation: "Associate professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: PKMishra,
      },
      {
        name: "Prof. Bharadwaj Nanda",
        designation: "Associate professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: BharadwajNanda,
      },
      {
        name: "Prof. Shiba Prasad Dash",
        designation: "Assistant professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: ShibaPDash,
      },
    ],
  },
  {
    committee: "Publicity Chair",
    committe_id: "publicity_chair",
    members: [
      {
        name: "Prof. Bibhuti Bhusan Mukharjee",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha",
        // image: BBMukherjee,
      },
      {
        name: "Prof. Atal Bihari Harichandan",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: AtalBHarichandan,
      },
      {
        name: "Prof.Prakash Kumar Panda",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: PrakashPanda,
      },
      {
        name: "Prof. Debashreet Das",
        designation: "Associate Professor",
        organization: "CUPGS, BPUT, Odisha, Rourkela",
        // image: DebashreetDas,
      },
    ],
  },
];

export const TpcMembers = [
  { name: "Abdul Khadar Shaik", organization: "Loyola Academy" },
  { name: "Ajit Kumar Senapati", organization: "Einstein Academy Of Technology And Management, Bhubaneswar" },
  { name: "Alpesh Kumar Dauda", organization: "Sambalpur University Institute of Information Technology" },
  { name: "Amar Ranjan Dash", organization: "Parala Maharaja Engineering College" },
  { name: "Anitha A", organization: "Vellore Institute Of Technology" },
  { name: "Arun Kumar", organization: "Nit Rourkela" },
  { name: "Asit Kumar Panda", organization: "Nist University" },
  { name: "Basanta Kumar Swain", organization: "Government College Of Engineering Kalahandi" },
  { name: "Bibudhendu Pati", organization: "Rama Devi Women's University, Bhubaneswar" },
  { name: "Bighnaraj Naik", organization: "Veer Surendra Sai University Of Technology, Burla" },
  { name: "Biswa Mohan Achrya", organization: "ITER, Siksha O Anusandhan Deemed To Be University" },
  { name: "Chandan Kumar Sahoo", organization: "Gandhi Institute Of Excellent Technocrats, Bhubaneswar" },
  { name: "Chandrakanta Mahanty", organization: "GITAM Deemed to be University, Visakhapatnam" },
  { name: "Chinmayee Padhy", organization: "Silicon Institute of Technology, Sambalpur " },
  { name: "Debabrata Dansana", organization: "Rajendra University, Balangir" },
  { name: "Debashreet Das", organization: "CUPGS, BPUT" },
  { name: "Deepak Kumar Rout", organization: "International Institute of Information Technology, Bhubaneswar" },
  { name: "Deepak Kumar Lal", organization: "Veer Surendra Sai University of Technology, Odisha" },
  { name: "Deepak Ranjan Biswal", organization: "Driems University" },
  { name: "Dillip Khamari", organization: "Vikash Institute of Technology Bargarh" },
  { name: "Dillip Kumar Biswal", organization: "Aryan Institute Of Engineering And Technology, Bhubaneswar" },
  { name: "Dinesh Kumar Dash", organization: "Parala Maharaja Engineering College" },
  { name: "Gopal Behera", organization: "Government College Of Engineering Kalahandi" },
  { name: "Gunamani Jena", organization: "Roland Institute Of Technology" },
  { name: "Hemant Kumar Apat", organization: "Kiit Deemed University" },
  { name: "Jibanananda Mehena", organization: "DRIEMS University,Cuttack" },
  { name: "Jui Pattnayak", organization: "Jis College Of Engineering" },
  { name: "K. Hemant Kumar Reddy", organization: "Vit-Ap University" },
  { name: "Kali Charan Rath", organization: "Gandhi Institute Of Engineering And Technology University, Gunupur" },
  { name: "Kakita Murali Gopal", organization: "Gandhi Institute Of Engineering And Technology University, Odisha" },
  { name: "Krishnaveni Bommidi", organization: "Gitam Deemed University" },
  { name: "Laxmi Sarat Chandra Nunnaguppala", organization: "IEEE" },
  { name: "Leena Samantaray", organization: "Ajay Binay Institute Of Technology" },
  { name: "M.Baliar Singh", organization: "Gayatri Institute Of Management" },
  { name: "Madhusmita Panda", organization: "Siskha O Anusandhan Deemed To Be University" },
  { name: "Malti Nagle", organization: "Pranveer Singh Institute Of Technology, Kanpur" },
  { name: "Manoj Kumar Patra", organization: "National Institute Of Technology Warangal" },
  { name: "Monalisa Mohanty", organization: "ITER, Siksha O Anusandhan University" },
  { name: "NILAMBAR Sethi", organization: "GIET University" },
  { name: "Niranjan Panigrahi", organization: "Parala Maharaja Engineering College Berhampur" },
  { name: "Om Prakash Jena", organization: "Ravenshaw University, Cuttack" },
  { name: "Pabitra Kumar Tripathy", organization: "Kalam Institute Of Technology" },
  { name: "Pavan Kumar", organization: "GIET UNIVERSITY, GUNUPUR" },
  { name: "Pradeep Kumar Biswal", organization: "IIIT Bhagalpur" },
  { name: "Prahallad Kumar Sahu", organization: "Gandhi Institute Of Engineering And Technology, University, Gunupur" },
  { name: "Prasenjit Dey", organization: "Nit Rourkela" },
  { name: "Priyadarsan Parida", organization: "Giet University" },
  { name: "Priyadarshi Kanungo", organization: "NISS institute of Engineering and Technology, Bhubaneswar" },
  { name: "Purna Chandra Sethi", organization: "Rama Devi Women's University, Bhubaneswar" },
  { name: "Purnendu Mishra", organization: "Vikash Institute Of Technology, Bargarh" },
  { name: "Pushkar Kishore", organization: "Kiit University" },
  { name: "Raghunandan Swain", organization: "Parala Maharaja Engineering College" },
  { name: "Rajendra Prasad Nayak", organization: "Government College Of Engineering Kalahandi Bhawanipatna" },
  { name: "Rajesh Kumar Behera", organization: "The Techno School (A Degree Engineering College), Bhubaneswar" },
  { name: "Rajiv Senapati", organization: "Srm University, Andhra Pradesh" },
  { name: "Ramesh Kumar Sahoo", organization: "Birla Global University, Bhubaneswar" },
  { name: "Ram Chandra Barik", organization: "C. V. Raman Global University" },
  { name: "Ranjan Keshari Pati", organization: "Modern Engineering And Management Studies" },
  { name: "Rashmi Ranjan Sahoo", organization: "Parala Maharaja Engineering College" },
  { name: "S. Gopal Krishna Patro", organization: "School Of Engineering, Sreenidhi University, Hyderabad" },
  { name: "Sakambari Mishra", organization: "Odisha University of Agriculture and Technology " },
  { name: "Sambit Kumar Mishra", organization: "Gandhi Institute For Education And Technology, Baniatangi" },
  { name: "Sanjaya Kumar Panda", organization: "Nit Warangal" },
  { name: "Sanjib Kumar Nayak", organization: "Veer Surendra Sai University Of Technology Burla" },
  { name: "Sandipan Sahu", organization: "Amity University Kolkata" },
  { name: "Santanu Kumar Dash", organization: "Vellore Institute Of Technology, Vellore, India" },
  { name: "Santosh Kumar Majhi", organization: "Guru Ghasidas Vishwavidyalaya, Bilaspur" },
  { name: "Santosh Kumar Satapathy", organization: "Pandit Deendayal Energy University" },
  { name: "Sarita Nayak", organization: "Suddhananda School Of Management & Computer Science" },
  { name: "Sasmita Acharya", organization: "Veer Surendra Sai University Of Technology" },
  { name: "Satyasundara Mahapatra", organization: "Pranveer Singh Institute Of Technology , Kanpur" },
  { name: "Siba Sankar Nayak", organization: "SILICON UNIVERSITY, ODISHA" },
  { name: "Sonalika Mishra", organization: "Silicon Institute of Technology, Sambalpur" },
  { name: "Souveek Roy", organization: "National Institute of Technology" },
  { name: "Srichandan Sobhanayak", organization: "Iiit Bhubaneswar" },
  { name: "Sruti Ranjan Mishra", organization: "Gandhi Institute For Education And Management, Baniatangi" },
  { name: "Subash Ranjan Kabat", organization: "Radhakrishna Institute Of Technology And Engineering" },
  { name: "Subasish Mohapatra", organization: "Odisha University of Technology and Research" },
  { name: "Subhrajit Pradhan", organization: "Gandhi Institute Of Excellent Technocrats" },
  { name: "Subhransu Sekhar Dash", organization: "Government College Of Engineering, Kalahandi" },
  { name: "Suman Kumar Das", organization: "University Of Brescia, Brescia, Italy" },
  { name: "Sumit Kar", organization: "Biet, Bhadrak" },
  { name: "Sunmya Das", organization: "Government College Of Engineering Kalahandi" }, // Corrected to "Soumya Das"
  { name: "Supra Swain", organization: "Siksha 'O' Anusandhan University, Iter, Bhubaneswar" }, // Corrected to "Sipra Swain"
  { name: "Sudhansu Kumar Mishra", organization: "BIRLA INSTITUTE OF TECHNOLOGY, RANCHI" },
  { name: "Sudheer Babu Punuri", organization: "Srm University, Andhra Pradesh" },
  { name: "Swarmila Subramanian", organization: "Astha School Of Management" }, // Corrected to "Sharmila Subramanian"
  { name: "Ugrabadi Sahoo", organization: "MITS, Deemed to be University, Andhra Pradesh" },
  { name: "Umashankar Ghugar", organization: "Srmist Delhi-Ncr Campus, Modinagar" },
  { name: "Yatindra Gopal", organization: "National Institute Of Technology, Arunachal Pradesh" },
];


export const trackChairMembers = [
  {
    committee: "Computer Science & Engineering",
    committe_id: "computer_science",
    members: [
      {
        name: "Arun Kumar",
        //   designation: "Hon'ble Vice Chancellor",
        organization: "NIT, Rourkela",
      },
    ],
  },
];
